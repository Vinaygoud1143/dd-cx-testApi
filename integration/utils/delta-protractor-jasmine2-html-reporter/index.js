...
function Jasmine2HTMLReporter(options) {
    ...
    function generateFilename(suite) {
        return getFullyQualifiedSuiteName(suite, true) + self.filePrefix + '.html';
    }
    ...
    function suiteAsHtml(suite) {
        ...
        html += '<li>Skipped: <strong>' + (Number(suite._skipped) + Number(suite._disabled)) + '</strong></li>';
        html += '<li>Failures: <strong>' + suite._failures + '</strong></li>';
        html += '<li>Text Execution Date and Time: <strong>' + moment().format("MM-DD-YYYY HH:mm A") + '</strong></li>';
        html += '</ul> </header>';
    ...
    function specAsHtml(spec) {
        ...
        if (spec.failedExpectations.length > 0 || spec.passedExpectations.length > 0) {
            ...
            _.each(spec.failedExpectations, function (expectation) {
                html += '<li>';
                html += '<span style="color:red;">' + expectation.message + '</span>' + '<span style="padding:0 1em;color:red;">&#10007;</span>';
                html += '</li></br>';
            });
            _.each(spec.passedExpectations, function (expectation) {
                html += '<li>';
                html += expectation.passed + '<span style="padding:0 1em;color:green;">&#10003;</span>';
                html += '</li></br>';
            });
            html += '</ul></div>';
        }
