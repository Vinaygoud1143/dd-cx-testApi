...
var TestData = require('../../../testData/devEng/' + testDataEnv + '/PaymentValidationWithDifferentCCTypes/ValidateCreditCardScenarios.json');
var TestDescriptions = require('../../../descriptions/devEng/PaymentValidationWithDifferentCCTypes/ValidateCreditCardScenarios_desc.json');

// D2C  Payment Service to check Difeferent Types Of Credit Card Validations".
describe('Credit Card Validate API With Difeferent Types Of Validate CreditCard Details:', function() {
    dataProvider(TestData.Scenarios, function(data, scenario) {
     ...
     it(scenario + ",Test Case 2:-" + TestDescriptions.ValidateCreditCardAPIScenarios[scenario]['TC-2'] + "", function() {
            ...
            if (resstatus == 200) {
                expect(respjson.encryptedToken).toEqual(data.verify['TC-2'].encryptedToken);
