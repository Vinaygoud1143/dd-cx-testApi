...
var TestData = require('../../../testData/devEng/' + testDataEnv + '/billingAndAccountAPI/BillingsAPI.json');
var TestDescriptions = require('../../../descriptions/devEng/billingAndAccountAPI/BillingsAPI_desc.json');

// Accounts & Billing Service - Billing API to get all billing details of given primary enrolleeID".
describe('Accounts & Billing API to display billing details of given primary enrolleeID :', function() {
    dataProvider(TestData.Scenarios, function(data, scenario) {
        ...
        it(scenario + ",Test Case 1:-" + TestDescriptions.Scenarios[scenario]['TC-1'] + "  PersonID: " + data.headers.PersonId + "  Client Key: " + data.headers.ClientKey + "", function(doneFn) {
            let apiurl = Utility.getapiurl('GETBILLINGDETAILS', data['path-params'].enrolleeId, '', data['query-params']);
            frisby
                .setup({
                    request: {
                        headers: {
