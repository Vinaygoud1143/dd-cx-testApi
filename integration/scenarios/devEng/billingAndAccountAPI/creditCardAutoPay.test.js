...
var TestData = require('../../../testData/devEng/' + testDataEnv + '/billingAndAccountAPI/creditCardAutoPay.json');
var TestDescriptions = require('../../../descriptions/devEng/billingAndAccountAPI/creditCardAutoPay_desc.json');

// Accoun
// ACCOUNT AUTO PAY With Credit Card Type And Checking The Valid Response STATUS".
describe('Accounts & Billing API to enable CC autopay :', function() {
    dataProvider(TestData.Scenarios, function(data, scenario) {
        ...
        it(scenario + ",Test Case 1:-" + TestDescriptions.Scenarios[scenario]['TC-1'] + "  PersonID: " + data.headers.PersonId + "  Client Key: " + data.headers.ClientKey + "", function(doneFn) {    
            let apiurl = Utility.getapiurl('ACCOUNTAUTOPAYCC', data['path-params'].enrolleeId, "", "");
